...
class BayesNetApproxInference(BayesNetReader):
    ...
    def __init__(self):
        if len(sys.argv) != 4:
            print("USAGE> BayesNetApproxInference_reject.py [your_config_file.txt] [query] [num_samples]")
            print("EXAMPLE> BayesNetApproxInference_reject.py \ config-heart-NaiveBayes.txt \P(target|sex=0,cp=3)\ 10000")
        else:
...
##            self.num_samples = int(1000)
            ...
            self.prob_dist = self.rejection_sampling(query_variable,evidence)
            print("probability_distribution="+str(self.prob_dist))
            
    def rejection_sampling(self,query_variable,evidence):
