...
class BayesNetApproxInference(BayesNetReader):
    ...
    def __init__(self):
        if len(sys.argv) != 4:
            print("USAGE> BayesNetApproxInference_reject _ test.py [your_config_file.txt] [your_test_file.csv] [num_samples]")
            print("EXAMPLE> BayesNetApproxInference_reject _ test.py \ config-heart-NaiveBayes.txt \ heart-data-discretized-test.csv \ 20000")
        else:
...
##            self.num_samples = int(20000)
    ...
    def rejection_sampling(self):
        ...
        for i in range(0, self.num_samples):
            X = self.prior_sample()
            if self.is_compatible_with_evidence(X, self.query_evidence):
        ...
        for key,value in C.items():
            value=self.pr+value
        if self.pr==0:
            print("Can not find any sample associated with test value")
        else:
